...
void User_add()
{
    ...
    Blank(inf_user);
    if(sex == 1) fwrite("男",s_c,3,inf_user);
    else fwrite("女",s_c,3,inf_user);
    Blank(inf_user);
...
void Single()
{
   ...
   // string ID;
    ...
    num_prz[0] = fgetc(inf_prz);    //读取奖品种类数
    ...
    for(i=0;i<n_prz;i++){   //循环抽奖品总数次
        ...
        for(j=0;j<n_p;j++){         //循环抽此奖品数量次
            ...
            fwrite(" | ",s_c,3,inf_hstr);
            fwrite("ID: ",s_c,4,inf_hstr);
            fwrite(ID,s_c,strlen(ID),inf_hstr);
            fwrite(" | ",s_c,3,inf_hstr);
            fwrite("用户名: ",s_c,11,inf_hstr);
            fwrite(name,s_c,strlen(name),inf_hstr);
            fwrite(" | ",s_c,3,inf_hstr);
            fwrite("奖品名: ",s_c,11,inf_hstr);
            fwrite(NAME,s_c,strlen(NAME),inf_hstr);
            Blank(inf_hstr);
...
void Show_set()
{
    ...
    Blank(inf_set);
    sprintf(answer,"%d",op1);
    fwrite(answer,s_c,strlen(answer),inf_set);
    Blank(inf_set);
    sprintf(answer,"%d",op2);
    fwrite(answer,s_c,strlen(answer),inf_set);
    Blank(inf_set);
...
int Input_B(string &input, int min_s, int max_s)
{
    ...
    while(1){
        ...
        len = input.length();
        //if(len == 0) return 0;
        if(len < min_s) illegal = 1;
        else if(len > max_s) illegal = 1;
